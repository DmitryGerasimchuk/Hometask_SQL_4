...
-- 0. Подключение БД, создание таблиц и заполнение таблиц данными (источник данных: https://drive.google.com/file/d/1PQn576YVakvlWrIgIjSP9YEf5id4cqYs/view)
...
-- Задание 1: Вывести на экран сколько машин каждого цвета для машин марок BMW и LADA
				-- Способ 1: сложный для написания, но простой для понимания: получаем таблицу с перечислением марок машин, соответсвующих цветов и их количеств

SELECT 
	MARK AS 'Марка машины',
	COLOR AS 'Цвет машины',
	COUNT(MARK) AS 'Количество машин'
FROM AUTO
...
WHERE MARK = 'LADA' AND COLOR = 'СИНИЙ';
				...
				--Способ 2: простой для написания: получаем суммарное количество машин BMW и LADA одного цвета, сгруппированное по цветам:
...
-- Задание 2: Вывести на экран марку авто и количество AUTO не этой марки (как я понял это задание: напротив маруи автомобиля вывести сумму автомобилей без учета этой марки)
SELECT 
	a1.MARK AS 'Марка машины',
	COUNT(a2.MARK) AS 'Количество машин не этой марки'
FROM (
...
-- Задание 3: Напишите запрос, который вернет строки из таблицы test_a, id которых нет в таблице test_b, НЕ используя ключевого слова NOT (надеюсь, IS NULL не будет приравнено к NOT).
CREATE TABLE TEST_A
(
	Id INT,
	Data_id VARCHAR(2)
